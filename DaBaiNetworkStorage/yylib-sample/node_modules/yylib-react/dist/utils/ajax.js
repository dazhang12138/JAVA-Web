...
var ajax = {
    ...
    , request: function request(options) {
        ...
        if (options.cookies === undefined) delete options.cookies;
        ...
        req.query(_defaults.query).send(_defaults.send).end(function (err, res) {
            if (err || res && res.badRequest) {
                ...
                if (typeof _defaults.error == 'function') {
                    ...
                    console.error('请求异常[状态码:' + err.status + ']', err);
            ...
            if (res && res.ok) {
                if (res.headers && res.headers['icop-content-type']) {
                    if (typeof _defaults.error == 'function') {
                        ...
                        //如果没有则使用内部默认的异常处理
                        //捕获服务端包装的异常消息
                        alert('异常错误\n' + (res.text && res.text.length > 0 ? res.text : res.body));
                        console.error('异常错误', res);
                        //YYNotification.error({
                        //    message: '错误',
